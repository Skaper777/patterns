const MAIN_CONAINER = document.querySelector(".recipes");
const SECTION = {
...
class Invoker {
  ...
  commands = [];
  // определяем, какие команды будут выполняться при запуске Execute()
  SetCommand(command) {
    this.commands.push(command);
  }
  Execute() {
    console.log("Invoker created", this.commands);
...
class Command {
  constructor() {
    if (this.constructor.name === "Command") {
      throw new Error(
        `${this.constructor.name}: can not create instance of interface`
      );
    }
  }
...
let getDataCommand = new GetDataCommand(jsonWorker);
invoker.SetCommand(getDataCommand);
invoker.Execute();
