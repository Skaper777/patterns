...
class Dish {
  ...
  chech(dishId) {
    const dishObject = this.areAvailable(dishId);

    if (dishObject?.COUNT > 0) {
      this.title = `${dishObject.title}<br> количество товара: ${dishObject.COUNT}`;
    } else if (this.nextObject) {
      this.nextObject.chech(dishId);
    } else {
      this.title = dishId.title || `Блюдо не найдено`;
    }

    return this.nextObject?.title || this.title;
  }
  setNext(nextObject) {
  ...
  areAvailable(dishId) {
    ...
    for (let vaults in LOCAL_BASE) {
      if (LOCAL_BASE[vaults].NAME === this.type) {
        LOCAL_BASE[vaults].PRODUCTS.forEach((product) => {
          if (product.ID === dishId) {
            result = product;
            result.title = LOCAL_BASE[vaults].NAME;
          }
        });
      }
    }
...
class Isabel extends Dish {
  constructor() {
    super();
    this.type = "ресторан 'Изабель'";
    return this;
  }
}

class Rapuncel extends Dish {
  constructor() {
    super();
    this.type = "кафе 'Рапунсель'";
    return this;
  }
}

const checkAvailability = (dishId) => {
  // Инициализируем источники
  const isabel = new Isabel();
  const rapuncel = new Rapuncel();

  isabel.setNext(rapuncel);
  //Активируем цепочку
  const result = isabel.chech(dishId);
...
const renderRecipes = (data) => {
  return `
    <li class='recipes__item'>
        <div class='recipes__inner'><img class='recipes__img' src='/static/images/content/${
          data.img
        }' alt='${data.title}'>
        ...
        <p class='recipes__description'>${data.discr}</p>
          <span class='recipes__ingredients'>${
            data.count_ing
          } ингредиентов</span>
